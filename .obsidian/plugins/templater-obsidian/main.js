...
var InternalModuleWeb = class extends InternalModule {
  ...
  get_weather() {
    return async (city = "Shanghai", params = "format=3") => {
      try {
  ...
  get_hitokoto(options = { c: void 0, encode: "json", charset: void 0, callback: void 0, select: void 0, min_length: void 0, max_length: void 0 }) {
    return async () => {
      try {
        ...
        if (options.c)
          url += `&c=${options.c}`;
        if (options.encode)
          url += `&encode=${options.encode}`;
        if (options.charset)
          url += `&charset=${options.charset}`;
        if (options.callback)
          url += `&callback=${options.callback}`;
        if (options.select)
          url += `&select=${options.select}`;
        if (options.min_length)
          url += `&min_length=${options.min_length}`;
        if (options.max_length)
          url += `&max_length=${options.max_length}`;
        const response = await this.getRequest(url);
        const json = await response.json();
...
> \u2014 ${author}`;
  ...
  generate_random_picture() {
    return async (size, query, include_size = false) => {
      try {
        const response = await this.getRequest(`https://templater-unsplash.fly.dev/${query ? "?q=" + query : ""}`);
        const json = await response.json();
        let url = json.full;
...
// wasm-embed:/workspaces/Templater/node_modules/@silentvoid13/rusty_engine/rusty_engine_bg.wasm
...
// toml:/workspaces/Templater/docs/documentation.toml
